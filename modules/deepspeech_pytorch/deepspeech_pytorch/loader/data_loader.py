...
class AudioParser(object):
    ...
    def compute_spectrogram(self, y):
        ...
        # STFT
        # print('NFFT', n_fft, 'hop', hop_length)
...
class MFCCParser(AudioParser):
    def __init__(self,
        ...
        :param augmentation_conf(Optional): Config containing the augmentation parameters
        """
        super(MFCCParser, self).__init__(audio_conf, normalize)
        self.aug_conf = augmentation_conf
        if augmentation_conf and augmentation_conf.noise_dir:
            self.noise_injector = NoiseInjection(path=augmentation_conf.noise_dir,
                                                 sample_rate=self.sample_rate,
        ...
        self.mfcc_transform = torchaudio.transforms.MFCC(sample_rate=self.sample_rate, n_mfcc=26, log_mels=True,
    ...
    def compute_mfcc(self, y):
        """
        :param y: Audio signal as an array of float numbers
        :return: mfcc of the signal"""

        return self.mfcc_transform(y)
...
class MFCCDataset(Dataset, MFCCParser):
    ...
    def __init__(self,
        ...
        super(MFCCDataset, self).__init__(audio_conf, normalize, aug_cfg)
...
def _collate_fn(batch):
    ...
    for x in range(minibatch_size):
        ...
        audio_dir.append(sample[2])
    targets = torch.tensor(targets, dtype=torch.long)
    return inputs, targets, input_percentages, target_sizes, audio_dir
