...
class DeepSpeech(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        inputs, targets, input_percentages, target_sizes,_ = batch
...
class DeepSpeechMFCC(pl.LightningModule):
    def __init__(self,
                 labels: List,
                 model_cfg: Union[UniDirectionalConfig, BiDirectionalConfig],
                 precision: int,
        ...
        self.conv = MaskConv(nn.Sequential(
            nn.Conv2d(1, 32, kernel_size=(1, 11), stride=(1, 2), padding=(0, 5)),
            nn.BatchNorm2d(32),
            nn.Hardtanh(0, 20, inplace=True),
            nn.Conv2d(32, 32, kernel_size=(1, 11), stride=(1, 1), padding=(0, 5)),
            nn.BatchNorm2d(32),
            nn.Hardtanh(0, 20, inplace=True)
        ))
        # Based on above convolutions and spectrogram size using conv formula (W - F + 2P)/ S+1
